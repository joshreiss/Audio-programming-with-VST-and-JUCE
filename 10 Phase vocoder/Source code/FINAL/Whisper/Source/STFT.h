...
public:
    ...
    STFT() : numChannels (1) { }
    virtual ~STFT() { }
    void setup (const int numInputChannels) { numChannels = (numInputChannels > 0) ? numInputChannels : 1; }
...
private:
    ...
    void updateHopSize (const int newOverlap) // should be 2, 4 or 8
    {
      DBG("new overlap" << newOverlap);
        overlap = newOverlap;
        if (overlap != 0) {
