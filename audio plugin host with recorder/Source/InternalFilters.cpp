                                                 ...
                                                 void (*callback) (void*, AudioPluginInstance*, const String&))
{
    ...
    if (desc.name == fileInDesc.name) {
        // one new shared data per player
        sharedData = new AudioPlayerPluginSharedData();
        p = new AudioPlayerPlugin();
        ((AudioPlayerPlugin*)p)->setSharedData(sharedData);
    }
    if (desc.name == fileOutDesc.name) {
        // recorders will get shared data from latest player
        p = new AudioSlaveRecorderPlugin();
        ((AudioSlaveRecorderPlugin*)p)->setSharedData(sharedData);
    }
...
void InternalPluginFormat::getAllTypes (OwnedArray<PluginDescription>& results)
{
    ...
    results.add (new PluginDescription (fileInDesc));
    results.add (new PluginDescription (fileOutDesc));
}
